# Инструкция по развёртыванию проекта BookCatalog на новом устройстве

---

## Требования

Перед началом убедись, что на устройстве установлено:

| Компонент | Где скачать |
|---|---|
| Visual Studio 2019 / 2022 | visualstudio.microsoft.com |
| Рабочая нагрузка **ASP.NET and web development** | При установке VS (галочка в установщике) |
| SQL Server LocalDB | Устанавливается автоматически вместе с VS |
| SSMS (SQL Server Management Studio) | aka.ms/ssmsfullsetup |

> LocalDB и SSMS можно скачать отдельно если Visual Studio уже установлена:
> **Tools → Get Tools and Features** внутри VS.

---

## Шаг 1. Перенести файлы проекта

Скопируй папку проекта на новое устройство любым удобным способом:
- флешка / внешний диск
- архив по почте или мессенджеру
- Google Drive / Яндекс Диск

Папка проекта находится по пути:
```
C:\Users\Ghost\source\repos\Book\BookCatalog\
```

Скопируй её целиком. На новом устройстве можно положить в любое место,
например:
```
...
3. Найди файл `BookCatalog.sln` внутри скопированной папки
4. Нажми **Open**
...
После открытия проекта Visual Studio может показать предупреждение
об отсутствующих пакетах. Чтобы их восстановить:

- Правый клик на решении в **Solution Explorer**
- Выбери **Restore NuGet Packages**

Подожди пока Visual Studio скачает все зависимости (нужен интернет).

---

## Шаг 4. Создать базу данных

### 4.1 Открыть SSMS и подключиться к LocalDB

1. Запусти **SQL Server Management Studio**
2. В поле **Server name** введи:
   ```
...
2. Скопируй и вставь весь скрипт из раздела **«Полный SQL-скрипт»** ниже
3. Нажми **F5** или кнопку **Execute**
4. В нижней части окна должны появиться результаты — таблица с числом
   созданных объектов

---

## Шаг 5. Проверить строку подключения

Открой файл `Web.config` в проекте и убедись, что строка подключения
выглядит так:
...
Если всё совпадает — ничего менять не нужно.

---

## Шаг 6. Запустить проект

Нажми **F5** или зелёную кнопку **IIS Express** в панели инструментов.

Браузер откроется автоматически. Если этого не произошло — перейди по адресу:
```
...
После запуска используй учётную запись администратора:

| Email | Пароль |
|---|---|
| `ad@test.com` | `123123` |

Если хочешь создать нового администратора — зарегистрируйся через форму
и выполни в SSMS:

```sql
USE BookCatalogDB;
UPDATE Users SET Role = 'Admin', MaxBookings = 10
...
Выполни этот скрипт **один раз** на новом устройстве в SSMS.

```sql
-- ============================================================
-- BookCatalog — создание базы данных, таблиц, процедур и данных
-- ============================================================
...
VALUES (N'Администратор', N'ad@test.com', N'123123', N'Admin', 10, 1);
GO
...
SELECT 'Stored procedures',
       COUNT(*) FROM sys.procedures WHERE name LIKE 'sp_%';
GO
```
...
**Ошибка «Cannot connect to (localdb)\MSSQLLocalDB»**
```bash
...
DROP DATABASE BookCatalogDB;
-- Затем выполни скрипт заново
```

**Ошибка компиляции в Visual Studio — пакеты не найдены**
- Правый клик на решении → **Restore NuGet Packages**
- Если не помогает: **Tools → NuGet Package Manager → Package Manager Console** → `Update-Package -reinstall`

**Страница не открывается после F5**
- Проверь что IIS Express запущен (иконка в трее)
- Попробуй открыть вручную: `https://localhost:44346`
