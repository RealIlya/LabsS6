...
<body>
    <form id="form1" runat="server">
        <asp:Label ID="lblDbStatus" runat="server" Visible="true" CssClass="info-message" Style="display:block; margin:10px 20px;" />
        ...
        <asp:Panel ID="pnlSearch" runat="server" CssClass="search-panel">
            ...
            <table class="search-form">
                <tr>
                    ...
                    <td><asp:TextBox ID="txtTitle" runat="server" CssClass="input" placeholder="Введите название..." /></td>
                </tr>
                <tr>
                    <td><asp:Label ID="lblAuthor" runat="server" Text="Автор:" AssociatedControlID="txtAuthor" /></td>
                    <td><asp:TextBox ID="txtAuthor" runat="server" CssClass="input" placeholder="Имя автора..." /></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td><asp:Label ID="lblYear" runat="server" Text="Год:" AssociatedControlID="txtYear" /></td>
                    <td>
                        <asp:TextBox ID="txtYear" runat="server" CssClass="input" placeholder="Год издания..." />
                        <asp:RangeValidator ID="rvYear" runat="server" ControlToValidate="txtYear" Type="Integer" MinimumValue="1000" MaximumValue="2100" ErrorMessage="Год должен быть в диапазоне 1000–2100" Display="Dynamic" EnableClientScript="true" CssClass="error" ValidationGroup="SearchGroup" />
                    </td>
                </tr>
            </table>
        ...
        <asp:Panel ID="pnlResults" runat="server" CssClass="results-panel">
            ...
            <asp:GridView ID="gvBooks" runat="server" AutoGenerateColumns="false" OnRowCommand="gvBooks_RowCommand" CssClass="books-grid" EmptyDataText="">
                ...
                <Columns>
                    <asp:BoundField DataField="Title" HeaderText="Название" ItemStyle-CssClass="col-title" />
                    <asp:BoundField DataField="Author" HeaderText="Автор" ItemStyle-CssClass="col-author" />
                    <asp:BoundField DataField="Year" HeaderText="Год" ItemStyle-CssClass="col-year" />
                    ...
                    <asp:TemplateField HeaderText="Доступно" ItemStyle-CssClass="col-available">
                        <ItemTemplate>
                            <asp:Label ID="lblAvailable" runat="server" Text='<%# Eval("AvailableCount") + " шт." %>' CssClass='<%# (int)Eval("AvailableCount") > 0 ? "status-available" : "status-unavailable" %>' />
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="Действия" ItemStyle-CssClass="col-actions">
                        <ItemTemplate>
                            <asp:LinkButton ID="lbDetails" runat="server" CommandName="ViewDetails" CommandArgument='<%# Eval("BookID") %>' Text="[ПОДРОБНЕЕ]" CssClass="action-link mono" CausesValidation="false" />
                            <asp:LinkButton ID="lbBook" runat="server" CommandName="Book" CommandArgument='<%# Eval("BookID") %>' Text="[ЗАБРОНИРОВАТЬ]" CssClass="action-link book-btn mono" CausesValidation="false" Visible='<%# (int)Eval("AvailableCount") > 0 %>' />
                        </ItemTemplate>
                    </asp:TemplateField>
                </Columns>
            </asp:GridView>
