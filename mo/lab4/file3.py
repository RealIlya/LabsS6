...
# --- 3. Одномерный поиск (ИСПРАВЛЕННЫЙ) ---
...
def line_search(x, d, r, tol=1e-5):
    # Нормализуем вектор d, чтобы start_step всегда означал одинаковое физическое расстояние!
    ...
    # Возвращаем шаг, адаптированный под оригинальную длину вектора d
    return res.x / d_norm
...
def method_cg_unconstrained(x0, r, tol=1e-4, max_iter=1000):
    # Увеличен max_iter и добавлена надежная логика Полака-Рибьера
    x = np.array(x0, dtype=float)
    n = len(x)
