...
def parabola_min(a, fa, b, fb, c, fc):
    """Вычисление минимума параболы по трём точкам"""
    return b - 0.5 * ((b - a)**2 * (fb - fc) - (b - c)**2 * (fb - fa)) / ((b - a) * (fb - fc) - (b - c) * (fb - fa))

def line_search(obj, x, d, tol=1e-5, max_iter=100):
    """Метод парабол (квадратичная интерполяция) для одномерной минимизации"""
    def phi(lam): return obj.evaluate(x + lam * d)

    # Шаг 1: Бреккетинг интервала унимодальности [a, c]
    a, c = bracket_unimodal(phi, start_step=1e-3, max_iter=100)
    
    # Выбираем среднюю точку b
    b = (a + c) / 2
    fa, fb, fc = phi(a), phi(b), phi(c)

    # Шаг 2: Метод парабол внутри [a, c]
    for _ in range(max_iter):
        denom = (b - a) * (fb - fc) - (b - c) * (fb - fa)
        if abs(denom) < 1e-16:
            break

        lam_new = parabola_min(a, fa, b, fb, c, fc)

        # Ограничиваем новой точкой внутри интервала
        lam_new = max(a + 1e-10, min(c - 1e-10, lam_new))

        # Проверка сходимости
        if abs(lam_new - b) < tol:
            return lam_new

        f_new = phi(lam_new)

        # Обновляем интервал, сохраняя унимодальность
        if lam_new < b:
            if f_new < fb:
                c, fc = b, fb
                b, fb = lam_new, f_new
