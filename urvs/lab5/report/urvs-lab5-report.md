...
Изучить механизмы межпроцессного взаимодействия на основе сегментов разделяемой памяти и аппарата семафоров.

Задание варианта 2: два дочерних процесса выполняют циклы работ и после завершения очередного цикла передают через один и тот же сегмент разделяемой памяти очередные четыре строки стихотворения; первый процесс передает нечетные четверостишия, второй — четные; циклы работ процессов не сбалансированы по времени; родительский процесс компонует из передаваемых фрагментов законченное стихотворение и выводит его после завершения работы обоих процессов; задача решается с использованием семафоров.

## 2. Анализ задачи

1. Создать сегмент разделяемой памяти для обмена одним сообщением между родителем и дочерними процессами.
2. Создать набор семафоров для синхронизации доступа к общему сегменту.
3. Породить процессы `P1` и `P2`.
4. Процесс `P1` передает четверостишия с номерами `1, 3, 5, 7`, процесс `P2` — `2, 4, 6, 8`.
5. Перед записью в сегмент разделяемой памяти дочерний процесс захватывает семафоры, после записи освобождает их и уведомляет родителя.
6. Родительский процесс считывает очередной фрагмент, помещает его в нужную позицию результирующего массива и освобождает сегмент для следующей передачи.
7. После получения всех фрагментов родитель выводит целое стихотворение и освобождает IPC-ресурсы.
...
| Функция | Назначение |
|:--|:--|
| `fork()` | Порождение дочерних процессов `P1` и `P2`. |
| `shmget()`, `shmat()`, `shmdt()`, `shmctl()` | Создание, подключение и удаление сегмента разделяемой памяти. |
| `semget()`, `semctl()`, `semop()` | Создание и использование набора семафоров для синхронизации. |
| `waitpid()` | Ожидание завершения дочерних процессов. |
| `kill()` | Аварийное завершение потомков при ошибке в родителе. |
| `nanosleep()` | Создание разной длительности циклов работы дочерних процессов. |
| `strtol()` | Разбор аргументов командной строки. |

## 4. Формат вывода результата

```text
P0(pid=...): created shared memory and semaphores
P1(pid=...): sent stanza 1
P0(pid=...): received stanza 1 from P1
...
...
- Рабочая директория: `C:\Users\Admin\Desktop\LabsS6\urvs\lab5`.
- Исходный файл: `c/main.c`.
- Платформа выполнения: `Linux/WSL`.
- Сборка: `gcc ./c/main.c -Wall -Wextra -Werror -std=c11 -o ./c/lab5.out`.
- Запуск: `./c/lab5.out [delay_child1_ms] [delay_child2_ms]`.
...
|:--|:--|:--|
| Позитивный запуск по умолчанию | `./c/lab5.out` | Корректная передача всех четверостиший, вывод стихотворения, код `0`. |
| Позитивный запуск с явными задержками | `./c/lab5.out 80 220` | Корректная работа при разной скорости дочерних процессов, код `0`. |
| Негативный запуск с нулевой задержкой | `./c/lab5.out 0` | Сообщение об ошибке в `stderr`, код `1`. |
| Негативный запуск с лишними аргументами | `./c/lab5.out 10 20 30` | Сообщение `usage` в `stderr`, код `1`. |
